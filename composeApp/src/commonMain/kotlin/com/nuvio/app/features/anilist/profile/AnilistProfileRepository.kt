...
object AnilistProfileRepository {
    ...
    suspend fun fetchFollowing(userId: Int, page: Int = 1): Result<List<AnilistSocialUserItem>> = withContext(Dispatchers.Default) {
        val users = AnilistApi.fetchFollowingUsers(userId = userId, page = page)
        Result.success(users)
    }

    suspend fun fetchFollowers(userId: Int, page: Int = 1): Result<List<AnilistSocialUserItem>> = withContext(Dispatchers.Default) {
        val users = AnilistApi.fetchFollowerUsers(userId = userId, page = page)
        Result.success(users)
    }
}
